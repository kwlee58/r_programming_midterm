...
(@) 공평한 주사위 두 개를 던져 합을 구하는 게임을 20번 반복하는 코드는 다음 중 어느 것인가? 위에서 정의한 `roll2`를 이용하시오. 
...
(@) `roll`함수를 편집하여 확률 조건을 추가한 `roll_w`를 만들려고 할 때 필요한 코드는? (단, 여기서 `;` 는 그 다음 코드를 이어서 수행하라는 것이다.)
...
```
url <- "https://raw.githubusercontent.com/coop711/r_programming_1/master/data/cards.csv"
